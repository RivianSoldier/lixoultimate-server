...
async def classify_image(data: JsonResult, db: Session = Depends(get_db)):
    ...
    if results and results[0].masks is not None:
        print(f"Found {len(results[0].masks)} segmentation masks to process.")
        for i, mask in enumerate(results[0].masks):
            box = results[0].boxes[i]
        ...
        print("Warning: No segmentation masks were found in the model results.")
    ...
    user = db.query(User).filter(User.id == data.userId).first()
    if not user:
        print(f"User {data.userId} not found, preparing new user for session.")
        user = User(id=data.userId, coins=0)
        db.add(user)
    else:
        print(f"User {user.id} found with {user.coins} coins.")
    ...
    try:
        ...
        print("✅ Successfully committed detection to database.")
        print(
            f"  -> Saved Data: id={detection_db_entry.id}, "
            f"status='{detection_db_entry.status}', "
            f"points_count={len(detection_db_entry.detection_points)}"
        )

    except Exception as e:
        db.rollback()
        print(f"!!! DATABASE ERROR ON FINAL COMMIT: {type(e).__name__} - {str(e)}")
        traceback.print_exc() 
        raise HTTPException(status_code=500, detail=f"Database error on final commit: {str(e)}")
